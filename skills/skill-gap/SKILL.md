...
Today is ${today}. Three fork-intelligence skills already exist: `fork-cohort` answers *is the fork alive?* (workflow runs in 7d), `fork-release` answers *has any fork shipped a versioned artifact?*, `contributor-spotlight` answers *who's pushing the most code?* None of them answer the obvious operator question: **"what's in upstream that I haven't adopted yet?"** This skill closes that layer.
...
The intent is: when fork-cohort is enabled and runs Sunday 19:00, skill-gap at 21:00 reuses its work. When fork-cohort hasn't been enabled yet, skill-gap still works, it's just slower.
...
Write `memory/topics/skill-gap-state.json`:
...
- **Compare on slug, not on enabled state.** `enabled: true` vs `enabled: false` is `fork-digest`'s job — this skill only answers "is the skill *present in the fork's skills.json* at all?" An enabled-but-stale skill is still a present skill here.
...
- The 200-char body truncation pattern from `fork-release` does not apply here — we don't read release bodies, only slug lists.
